...
글은 `src/pages/PartnersPage.tsx` 에 그대로 적혀 있다. 히어로 배경은 사진이
아직 없어서 `home.css` 의 `.partners__hero` 가 CSS 로 그리고 있다. 사진이
준비되면 다른 화면과 같은 방식으로 얹으면 된다.

1. `scripts/build-images.mjs` 의 `IMAGES` 에 가로용·세로용 두 줄을 더한다
   (`partners-wide` / `partners-tall`).
2. `node scripts/build-images.mjs <원본 폴더>` 를 돌린다.
3. `PartnersPage.tsx` 의 주석이 가리키는 자리에 `<Photo>` 와 `.hero2__veil` 을 넣는다.
